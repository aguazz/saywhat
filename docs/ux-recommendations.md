...
### Ultimate Debate Visualization (Prompts A–D) ✓

A full redesign of the Thread Timeline sub-tab into the primary debate exploration surface.

---

#### Prompt A — Color-coding toggle ✓

**Location:** Thread Timeline sub-tab, above the legend.

**What was done:** A compact three-button toggle (`[Speakers] [Stage] [Claim type]`) sits above the timeline lanes. Clicking a button rewrites the bar colors across all lanes without any pipeline call:

| Mode | Colors | Legend |
|------|--------|--------|
| Speakers (default) | Per-speaker palette (colored circles) | Circles + speaker names |
| Stage | red=Confrontation, blue=Opening, green=Argumentation, purple=Concluding | Colored squares + stage names |
| Claim type | blue=Factual/Statistical, orange=Causal/Predictive, purple=Definitional, red=Moral | Colored squares + type group names |

Active mode shown as a filled (primary) button. Stage mode disabled until Analyze Rhetoric has run. Color mode stored in `st.session_state["tl_color_mode"]`. Module-level `STAGE_COLORS` and `TYPE_COLOR_MAP` constants added. `_build_thread_timeline()` extended with `color_mode` and `claim_stage_map` parameters. 8 new bilingual LABELS entries.

---

#### Prompt B — 60/40 timeline | claim card split ✓

**Location:** Thread Timeline sub-tab.

**What was done:** The sub-tab now uses `st.columns([3, 2])`. The left 60% holds the clickable timeline visualization; the right 40% holds a persistent claim card panel (`st.container(border=True)`). When no claim is selected the right panel shows a short hint; when a claim is selected the card appears instantly without scrolling.

The claim card rendering was extracted into a module-level `_render_claim_card(sel_c, tl_claims, tl_threads, speaker_names, lang, api_key)` function, replacing ~300 lines of deeply-nested inline code and making the card reusable for future surfaces.

Mini-map height reduced 320 → 250 px to fit the narrower panel. Scorecards, toggle, and legend remain full-width above the columns. 1 new bilingual LABELS entry: `card_empty_hint`.

---

#### Prompt C — Purpose section: AI role sentence + stance ✓

**Location:** Claim card (right panel), between the metadata line and the type badges.

**What was done:** Two new elements appear after Speaker · Time · Thread · Stage:

1. **Stance badge** — shown only when a motion is set. ▲ (green) = for the motion, ▼ (red) = against, ● (grey) = neutral. Uses existing stance field from the extractor.

2. **Role sentence** — a ≤12-word phrase describing the argumentative role of this claim, generated by Claude Haiku on first click and cached in `st.session_state["claim_roles"][claim_id]`. Responds in the debate's language automatically. While loading a spinner shows; failed calls cache `""` to prevent retry loops.

New module-level function: `_get_claim_role(claim, thread_topic, n_connections, api_key) → str`. 3 new bilingual LABELS entries: `card_purpose`, `card_role_loading`, `card_stance_label`.

---

#### Prompt D — "Open in full map" button ✓

**Location:** Claim card (section 5.5, below the mini-map toggle), and Argument Map sub-tab.

**What was done:**

- **Claim card**: "Open in full map" button appears below the mini-map whenever the claim has direct connections. Clicking stores `{sel_id} ∪ neighbour_ids` in `st.session_state["map_focus_ids"]` and reruns.

- **Argument Map sub-tab**: on each render, checks `map_focus_ids`. If set, overrides `_hi_ids` so the graph fades all non-neighbourhood nodes and edges. A caption "Showing neighbourhood from Thread Timeline." + "✕ Clear focus" button appear above the graph; clearing removes `map_focus_ids` and restores normal filter state.

The user switches to the Argument Map tab manually; the tab detects the focus state automatically. 3 new bilingual LABELS entries: `card_open_in_map`, `map_focus_caption`, `map_focus_clear`.
